...
public class NameTag : NetworkBehaviour {
	...
	[ClientCallback]
	void OnGUI() {
		if (!player.clientController.isLocalPlayer) {
			...
			Rect textCentered = new Rect(pos.x - labelSize.x / 2, Screen.height - pos.y - labelSize.y + 4, labelSize.x,
				labelSize.y);
	...
	private void onSetName(string name) {
		displayName = name;
		if (!enabled && !player.clientController.isLocalPlayer)
			enabled = true;
	}
}
